...
def supply_bak(height):
    ...
    return {
        "halvings": int(0),
        "supply": satoshis(int(data2["result"]["total_amount"])),
        "total/max supply": satoshis(18000000)
        #"supply": type(str(sub_total_supply) + "00000000")
    }
    ...
    try:
        try:
            result = subprocess.run(
                ['python3', '/root/Projects/Nusa/ApiServer/gen_price.py'],
                check=True,
                capture_output=True,
                text=True
            )
            ...
            usd = 0.0
        try:
            with open('/root/Projects/Nusa/ApiServer/price.log', 'r') as file:
            ...
            print("The file price.log was not found.")
...
def getprice2():
    url = "https://api.coinpaprika.com/v1/tickers/cicx-cicoin"
    try:
