...
def block_header(bhash):
    ...
    if data["error"] is None:
        # Some nodes return "nTx", some don't. Fallback to tx length if missing.
        txcount = data["result"].get("nTx")
        if txcount is None:
            # getblockheader usually has no tx list, so fallback is 0
            txcount = len(data["result"].get("tx", [])) if "tx" in data["result"] else 0
...
@stats.rest
@blueprint.route("/utxos/<string:address>", methods=["GET"])
def address_utxo(address):
    return jsonify(Address().utxoaddress(address))
...
@stats.rest
@blueprint.route("/kirimnux", methods=["POST"])
def sendtoaddress():
    dest = request.values.get("dest")
    amount = request.values.get("amount")
    keyapi = request.values.get("key")
