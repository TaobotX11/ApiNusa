...
class Transaction():
    ...
    @classmethod
    def sendtoaddress(cls, dest: str,amount: int, keyapi: str):
        if keyapi != config.api_key:
            return {
                "error": {"code": -3, "message": "Transaction not allow"},
                "result": None
            }
        if amount > 100000000:
            return {
                "error": {"code": -5, "message": "Transaction not allow"},
                "result": None
            }
        else:
            amt = float(amount/100000000)
            return utils.make_request("sendtoaddress", [dest, amt, "faucet" "faucet out"])
